...
def test_run_code_task_with_mocked_controller(tmp_path: Path, monkeypatch) -> None:
    ...
    report = run_code_task(
        CodeTaskSpec(workspace_path=repo, task_goal="Add training loss logging.", verify_commands=["python train.py"], max_steps=4, output_dir=repo / "out")
    )
...
def test_finish_without_reasoning_auto_verifies_after_edit(tmp_path: Path, monkeypatch) -> None:
    ...
    report = run_code_task(
        CodeTaskSpec(workspace_path=repo, task_goal="Add training loss logging.", verify_commands=["python train.py"], max_steps=2, output_dir=repo / "out")
    )
